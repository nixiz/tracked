...
template<class Type, class Deleter = dtl::default_deleter<Type>,
...
class tracked_ptr final : public Options<typename Traits::exception_policy>... {
 public:
  ...
  using pointer          = typename dtl::get_deleter_pointer_type<Type, std::remove_reference_t<deleter_type>>::type;
  ...
  template<class _Dx2 = deleter_type, std::enable_if_t<std::is_constructible_v<_Dx2, const _Dx2&>, int> = 0>
  ...
  template<
      class _Dx2 = deleter_type,
      std::enable_if_t<
          std::conjunction_v<std::is_reference<_Dx2>, std::is_constructible<_Dx2, std::remove_reference_t<_Dx2>>>, int> = 0>
  ...
  template<
      ...
      std::enable_if_t<std::conjunction_v<std::negation<std::is_array<_Ty2>>,
                                          std::is_convertible<typename tracked_ptr<_Ty2, _Tr2, _Op2...>::pointer, pointer>,
                                          std::conditional_t<std::is_reference_v<deleter_type>,
                                                             std::is_same<typename _Tr2::deleter, deleter_type>,
                                                             std::is_convertible<typename _Tr2::deleter, deleter_type>>>,
                       int> = 0>
  tracked_ptr(tracked_ptr<_Ty2, _Tr2, _Op2...>&& _Right) noexcept
      : my_pair(dtl::one_then_variadic_args_t(), std::forward<typename _Tr2::deleter>(_Right.get_deleter()),
                _Right.release())
  {
  }
  ...
  tracked_ptr& operator=(tracked_ptr&& _Right) noexcept
  {
    ...
    if (this != std::addressof(_Right)) {
      ...
      my_pair.get_deleter() = std::forward<deleter_type>(_Right.my_pair.get_deleter());
    }
    return *this;
  }
  ...
  {
    if (my_pair.pointer) {
      (Options<exception_policy>::destruct(), ...);
  ...
  [[nodiscard]] std::add_lvalue_reference_t<Type> operator*() const noexcept(tracked_ptr::is_noexcept) /* strengthened */
  {
    // call every options apply method.
    (Options<exception_policy>::dereference(), ...);
